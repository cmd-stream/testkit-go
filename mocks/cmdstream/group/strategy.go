...
type (
	NextFn[T any]  = func() (grp.Client[T], int64)
	SliceFn[T any] = func() []grp.Client[T]
)
...
func (f DispatchStrategy[T]) Next() (client grp.Client[T], index int64) {
	result, err := f.Call("Next")
	if err != nil {
		panic(err)
	}
	client = result[0].(grp.Client[T])
	index = result[1].(int64)
	return
}

func (f DispatchStrategy[T]) Slice() []grp.Client[T] {
	result, err := f.Call("Slice")
	if err != nil {
		panic(err)
	}
	return result[0].([]grp.Client[T])
}
